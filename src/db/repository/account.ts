...
const columns = "id, name, type, balance, currency, icon, color, is_default AS isDefault, created_at AS createdAt, updated_at AS updatedAt";

export async function createAccount(data: AccountInput, tx: any = db) {
  try { const now = Date.now(); await tx.runAsync("INSERT INTO accounts (id,name,type,balance,currency,icon,color,is_default,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?)", randomUUID(), data.name, data.type, data.balance, data.currency, data.icon ?? null, data.color ?? null, data.isDefault ? 1 : 0, now, now); }
...
export async function updateAccount(id: string, data: AccountInput, tx: any = db) { try { await tx.runAsync("UPDATE accounts SET name=?, type=?, balance=?, currency=?, icon=?, color=?, is_default=?, updated_at=? WHERE id=?", data.name, data.type, data.balance, data.currency, data.icon ?? null, data.color ?? null, data.isDefault ? 1 : 0, Date.now(), id); } catch (error) { throw new Error(`Unable to update account: ${String(error)}`); } }
...
export async function getTransactionCountForAccount(id: string, tx: any = db) { try { const row = await tx.getFirstAsync("SELECT COUNT(*) AS count FROM transactions WHERE account_id = ?", id) as { count: number } | null; return row?.count ?? 0; } catch (error) { throw new Error(`Unable to count account transactions: ${String(error)}`); } }
